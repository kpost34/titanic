...
```
The col_types argument matches the data types in the csv file (e.g., i=integer, f=factor, c=character, n=number), except for the variable Embarked, which is explained below.
\
\
...
### Data cleaning
The names and order of the columns were changed. The levels of the pclass factor were re-ordered. The embarked variable was coded as a factor. This strategy solved the problem of read_csv() creating an empty factor level when designating the variable as a factor.
...
```
Now the pclass levels are in numerical order, and the embarked levels are correct.
\
\
